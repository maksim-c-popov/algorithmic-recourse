...
def loadSCM(scm_class, experiment_folder_name = None):
  ...
  if scm_class == 'sanity-3-lin':
    ...
    structural_equations_ts = structural_equations_np
    noises_distributions = {
      'u1': MixtureOfGaussians([0.5, 0.5], [1, 3], [1, 1]),
      'u2': Normal(0, 2),
      'u3': Normal(0, 2),
    }
  ...
  assert \
    list([utils.getNoiseStringForNode(node) for node in structural_equations_np.keys()]) == \
    list([utils.getNoiseStringForNode(node) for node in structural_equations_ts.keys()]) == \
